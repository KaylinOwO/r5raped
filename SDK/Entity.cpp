...
DWORD64 CBaseEntity::GetWeapon()//ÔÝÊ±ÓÃDWORD64
{
	if (!this)
		return 0;
...
int CBaseEntity::GetWeaponID()//ÔÝÊ±ÓÃDWORD64
{
	if (!this)
		return -1;
...
string CBaseEntity::GetPlayerName()
{
	if (!this)
		return "";
	int entityId = CUtils::Read<int>((DWORD64)this + OFFSET::s_entityID);		//ItemID0x9C		//ÕâÀïµÄ30ºÍAOO Í¨¹ýÌØÕ÷Âë¶¨Î»ÊÖ¶¯¸üÐÂ
...
void CBaseEntity::StartShot(bool bSwitch)
{
	if (!this)
		return;
	if ( bSwitch)
		*(DWORD64*)(global::g_hGameImage + OFFSET::g_AutoShotAddr) = 5;	//µÈÓÚ5ÊÇ°´ÏÂ
	else
